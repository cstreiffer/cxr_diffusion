...
def find_best_checkpoint(directory, epoch=None):
    ...
    print(f"Loading file {best_file}")
    return os.path.join(directory, best_file)

def run_test_set(checkpoint_fp, test_set_settings_path, file_name=""):

    checkpoint = torch.load(checkpoint_fp, map_location=torch.device("cuda"))
    tst = torch.load(test_set_settings_path, map_location=torch.device("cuda"))
